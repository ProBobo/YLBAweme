...
- (instancetype)init {
    if (self = [super init]) {
        ...
        [self setBackgroundImage:[UIImage gk_imageWithColor:[UIColor clearColor] size:CGSizeMake(YLB_SCREEN_WIDTH, YLBTabBarHeight)]];
        [self showLine];
    }
    return self;
}
...
- (void)showLine {
    self.shadowImage = [UIImage gk_imageWithColor:[UIColor colorWithWhite:1.0f alpha:0.2f] size:CGSizeMake(YLB_SCREEN_WIDTH, 0.5f)];
}

- (void)hideLine {
    self.shadowImage = [UIImage gk_imageWithColor:[UIColor clearColor] size:CGSizeMake(YLB_SCREEN_WIDTH, 0.5f)];
}
